...
def log_model(
    model: Any, signature, model_name: str = "model", conda_env: Optional[Dict] = None
) -> None:
    """Log a model to MLflow."""
    mlflow.lightgbm.log_model(
        lgb_model=model, signature=signature, name=model_name, conda_env=conda_env
    )
...
def log_optuna_study(study) -> None:
    ...
    if not completed_trials:
        print("Warning: No completed trials to log")
        return
    ...
    try:
        ...
        try:
            ...
            log_figure(
                figure=fig,
                figure_name="optimization_history",
                artifact_path="optuna_plots",
            )
            ...
            if len(completed_trials) >= 2:
                ...
                log_figure(
                    figure=fig,
                    figure_name="parallel_coordinate",
                    artifact_path="optuna_plots",
                )
        ...
        print(f"Warning: Could not log best trial information: {e}")
